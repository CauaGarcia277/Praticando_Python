...
# que é obtida multiplicando a quantidade pelo preço unitário. Além disso, a tabela precisa conter 
...
total = [multiplicacao(quantidade[i], preco[i]) for i in range(len(id))]

cabecalho = ('id', 'quantidade', 'preco', 'total')
lista = [(id[i], quantidade[i], preco[i], total[i]) for i in range(len(id))]
